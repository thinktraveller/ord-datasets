# ChemRxiv Aniline-Amidation Yield (%) / ChemRxiv 苯胺酰胺化产率（%）
...
[`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv) |
| Included / excluded / input records | 957 / 3 / 960 |
...
Yield modelling for amidation of a fixed carboxylic-acid core with varied anilines. It chiefly compares amine substrates, activating agents, bases, and solvent combinations to identify conditions that transfer across aniline substrates.
...
- Observed exclusions: `label_out_of_range`: 3 record(s)
...
| File | Purpose |
|---|---|
| [`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
...
- `ord_dataset-7acd6ad2bf4d4cff841cad008ab726d5`: [data/7a/ord_dataset-7acd6ad2bf4d4cff841cad008ab726d5.parquet](https://github.com/open-reaction-database/ord-data/blob/83f971f586f6ad18f358ae4ae99d045e94ed2066/data/7a/ord_dataset-7acd6ad2bf4d4cff841cad008ab726d5.parquet)  (ORD revision `83f971f586f6ad18f358ae4ae99d045e94ed2066`; SHA-256 `b6dfe8e151a396c5332668c015bce764d23c667240e443495990205c66e4a1e5`)
...
[`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv)；它的内容与不可变
[`v0.1.0-model-ready-preview-1`](https://github.com/thinktraveller/ord-datasets/releases/tag/v0.1.0-model-ready-preview-1)
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 957 / 3 / 960 |
| 标签列 | `yield_percent` |
...
| 试剂、催化剂、溶剂等 | `reagent-1`、`reagent-2`、`catalyst-1`、`catalyst-2`、`solvent-1`、`solvent-2`、`solvent-3` |
| 产物 | `product` |
| 条件 | `temperature_c`、`reaction_time_s`、`reflux_value`、`conditions_are_dynamic_value` |
| 标签 | `yield_percent` |
...
| 文件 | 用途 |
|---|---|
| [`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`chemrxiv-aniline-amidation-yield-percent-dataset.csv`](chemrxiv-aniline-amidation-yield-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
