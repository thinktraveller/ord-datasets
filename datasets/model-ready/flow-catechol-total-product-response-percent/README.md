# Flow Catechol Total-Product HPLC Response (%) / 连续流儿茶酚总产物 HPLC 响应（%）
...
[`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv) |
| Included / excluded / input records | 1,227 / 0 / 1,227 |
| Label column | `total_product_yield_percent` |
...
Research-only flow solvent-selection target for the sum of Product 2 and Product 3 percentage responses. The label is their deterministic sum; reliable run grouping is not yet available.
...
| Label | `total_product_yield_percent` |
...
| File | Purpose |
|---|---|
| [`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
...
[`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv)；它的内容与不可变
[`v0.1.0-model-ready-preview-1`](https://github.com/thinktraveller/ord-datasets/releases/tag/v0.1.0-model-ready-preview-1)
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 1,227 / 0 / 1,227 |
| 标签列 | `total_product_yield_percent` |
...
研究型 target：面向 Product 2 与 Product 3 百分比响应之和的流动溶剂选择建模。标签由两个产物响应确定性相加得到，重点考察溶剂配比和流动条件；当前不具备可靠的运行分组界定。
...
| 标签 | `total_product_yield_percent` |
...
| 文件 | 用途 |
|---|---|
| [`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`flow-catechol-total-product-response-percent-dataset.csv`](flow-catechol-total-product-response-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
