# Suzuki-Miyaura Nanoscale Yield (%) / Suzuki-Miyaura 纳米尺度产率（%）
...
[`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv) |
| Included / excluded / input records | 5,760 / 0 / 5,760 |
...
| Associated publication | [10.1126/science.aap9112](https://doi.org/10.1126/science.aap9112) |
...
Percent-yield modelling for Suzuki-Miyaura C-C coupling. Aryl electrophiles and organoboron nucleophiles are combined systematically with ligands, bases, solvents, and catalytic conditions at small scale, supporting study of structure-condition interactions.
...
| Reagents, catalysts, solvents, and other components | `reagent-1`, `catalyst-1`, `catalyst-2`, `solvent-1`, `solvent-2`, `solvent-3`, `solvent-4`, `solvent-5`, `solvent-6`, `solvent-7` |
| Products | `product` |
| Conditions | `temperature_c`, `pressure_kpa`, `reaction_time_s`, `conditions_are_dynamic_value` |
| Label | `yield_percent` |
...
| File | Purpose |
|---|---|
| [`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
...
- `ord_dataset-68cb8b4b2b384e3d85b5b1efae58b203`: [data/68/ord_dataset-68cb8b4b2b384e3d85b5b1efae58b203.parquet](https://github.com/open-reaction-database/ord-data/blob/83f971f586f6ad18f358ae4ae99d045e94ed2066/data/68/ord_dataset-68cb8b4b2b384e3d85b5b1efae58b203.parquet)  (ORD revision `83f971f586f6ad18f358ae4ae99d045e94ed2066`; SHA-256 `36cea7705307a5e359a176ed3862994e3edc60ae4363fc2f3c5862affff6bb21`)
...
[`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv)；它的内容与不可变
[`v0.1.0-model-ready-preview-1`](https://github.com/thinktraveller/ord-datasets/releases/tag/v0.1.0-model-ready-preview-1)
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 5,760 / 0 / 5,760 |
| 标签列 | `yield_percent` |
...
| 数据许可证 | `CC-BY-SA-4.0` |
| 相关论文 | [10.1126/science.aap9112](https://doi.org/10.1126/science.aap9112) |

> “model-ready”表示文件可以直接读取和建模，并不表示它已被接受为通用基准。建模或比较前，请结合上表的就绪状态与标签含义判断是否适合您的问题。

### 化学范围与筛选

面向 Suzuki–Miyaura C–C 偶联的百分比产率建模。体系以芳基亲电体和有机硼亲核体为核心，系统覆盖配体、碱、溶剂与催化条件的组合；反应规模小、条件组合密集，适合研究结构与条件的交互效应。
...
| 试剂、催化剂、溶剂等 | `reagent-1`、`catalyst-1`、`catalyst-2`、`solvent-1`、`solvent-2`、`solvent-3`、`solvent-4`、`solvent-5`、`solvent-6`、`solvent-7` |
| 产物 | `product` |
| 条件 | `temperature_c`、`pressure_kpa`、`reaction_time_s`、`conditions_are_dynamic_value` |
| 标签 | `yield_percent` |
...
| 文件 | 用途 |
|---|---|
| [`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`suzuki-miyaura-nanoscale-yield-percent-dataset.csv`](suzuki-miyaura-nanoscale-yield-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
