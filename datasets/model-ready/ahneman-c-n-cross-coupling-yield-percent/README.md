# Ahneman C-N Cross-Coupling Yield (%) / Ahneman C-N 交叉偶联产率（%）

这是一个可直接用于反应建模的 ORD 数据子集。主表为
[`ahneman-c-n-cross-coupling-yield-percent-dataset.csv`](ahneman-c-n-cross-coupling-yield-percent-dataset.csv)；它的内容与不可变
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`ahneman-c-n-cross-coupling-yield-percent-dataset.csv`](ahneman-c-n-cross-coupling-yield-percent-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 4,312 / 0 / 4,312 |
| 标签列 | `yield_percent` |
...
| 数据许可证 | `CC-BY-SA-4.0` |
| 相关论文 | [10.1126/science.aar5169](https://doi.org/10.1126/science.aar5169) |

> “model-ready”表示文件可以直接读取和建模，并不表示它已被接受为通用基准。建模或比较前，请结合上表的就绪状态与标签含义判断是否适合您的问题。

## 化学范围与筛选

面向 Buchwald–Hartwig 型 C–N 交叉偶联的百分比产率建模。数据同时改变芳基底物、胺底物、配体、碱、钯催化剂和溶剂，适合比较底物泛化与催化体系选择对偶联效率的影响。
...
| 试剂、催化剂、溶剂等 | `reagent-1`、`reagent-2`、`catalyst-1`、`solvent-1`、`solvent-2`、`solvent-3`、`solvent-4`、`solvent-5` |
| 产物 | `product` |
...
| 文件 | 用途 |
|---|---|
| [`ahneman-c-n-cross-coupling-yield-percent-dataset.csv`](ahneman-c-n-cross-coupling-yield-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`ahneman-c-n-cross-coupling-yield-percent-dataset.csv`](ahneman-c-n-cross-coupling-yield-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
