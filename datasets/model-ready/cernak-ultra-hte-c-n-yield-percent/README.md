# Cernak Ultra-HTE C-N Yield (%)

[中文](README-zh.md)

This is an ORD subset prepared for direct reaction modelling. Its main table is
[`cernak-ultra-hte-c-n-yield-percent-dataset.csv`](cernak-ultra-hte-c-n-yield-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`cernak-ultra-hte-c-n-yield-percent-dataset.csv`](cernak-ultra-hte-c-n-yield-percent-dataset.csv) |
| Included / excluded / input records | 50,668 / 20 / 50,688 |
...
| Associated publication | [10.1021/jacs.6c05959](https://doi.org/10.1021/jacs.6c05959) |
...
Assay-yield modelling for Pd-, Ni-, and Cu-catalysed C-N coupling. This high-throughput system varies aryl halides, amines, metal/ligand combinations, bases, temperatures, and solvents across a broad chemical space.
...
- Observed exclusions: `label_out_of_range`: 20 record(s)
...
| Reagents, catalysts, solvents, and other components | `reagent-1`, `catalyst-1`, `catalyst-2`, `solvent-1`, `solvent-2`, `solvent-3`, `solvent-4`, `solvent-5`, `solvent-6`, `solvent-7`, `solvent-8`, `solvent-9`, `solvent-10` |
| Products | `product` |
...
| File | Purpose |
|---|---|
| [`cernak-ultra-hte-c-n-yield-percent-dataset.csv`](cernak-ultra-hte-c-n-yield-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`cernak-ultra-hte-c-n-yield-percent-dataset.csv`](cernak-ultra-hte-c-n-yield-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
