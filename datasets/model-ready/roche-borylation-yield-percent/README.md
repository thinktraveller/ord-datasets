# Roche Borylation Yield (%)

[中文](README-zh.md)

This is an ORD subset prepared for direct reaction modelling. Its main table is
[`roche-borylation-yield-percent-dataset.csv`](roche-borylation-yield-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`roche-borylation-yield-percent-dataset.csv`](roche-borylation-yield-percent-dataset.csv) |
| Included / excluded / input records | 890 / 221 / 1,111 |
...
| Associated publication | [10.26434/chemrxiv-2023-nfq7h](https://doi.org/10.26434/chemrxiv-2023-nfq7h) |
...
Percent-yield modelling for organoborylation reactions. Structured records contain reactants, reagents, catalysts, and solvents, supporting analysis of how functional groups and condition combinations affect borylation outcomes.
...
- Observed exclusions: `label_out_of_range`: 2 record(s); `missing_desired_product`: 219 record(s)
...
| File | Purpose |
|---|---|
| [`roche-borylation-yield-percent-dataset.csv`](roche-borylation-yield-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`roche-borylation-yield-percent-dataset.csv`](roche-borylation-yield-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
