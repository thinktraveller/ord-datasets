# NiCOlit Nickel-Coupling Yield (%) / NiCOlit 镍催化偶联产率（%）
...
[`nicolit-nickel-coupling-yield-percent-dataset.csv`](nicolit-nickel-coupling-yield-percent-dataset.csv)；它的内容与不可变
[`v0.1.0-model-ready-preview-1`](https://github.com/thinktraveller/ord-datasets/releases/tag/v0.1.0-model-ready-preview-1)
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`nicolit-nickel-coupling-yield-percent-dataset.csv`](nicolit-nickel-coupling-yield-percent-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 1,669 / 93 / 1,762 |
| 标签列 | `yield_percent` |
| 标签含义与单位 | percent yield as defined by the source measurement |
| 标签选择规则 | `unique_structured_scalar_no_aggregation` |
| 模型/基准就绪状态 | `generated_with_source_caveat` |
| 数据许可证 | `CC-BY-SA-4.0` |
| 相关论文 | [10.1002/chem.201603436](https://doi.org/10.1002/chem.201603436) |

> “model-ready”表示文件可以直接读取和建模，并不表示它已被接受为通用基准。建模或比较前，请结合上表的就绪状态与标签含义判断是否适合您的问题。

### 化学范围与筛选

面向镍催化有机锂交叉偶联的百分比产率建模，涉及醚或芳基铵盐的 C–O/C–N 键活化。标签保留零值；不同反应条目的 GC 与分离产率口径未区分，使用时应将其视为解释限制。

- 本目标只使用下方列出的 ORD 物理数据源。
- 只有满足 `unique_structured_scalar_no_aggregation` 的标签记录被纳入主表；不会对多个候选标签做未声明的平均或聚合。
- 实际排除情况：`missing_desired_product`：92 条；`missing_required_reactant_smiles`：1 条
...
| 文件 | 用途 |
|---|---|
| [`nicolit-nickel-coupling-yield-percent-dataset.csv`](nicolit-nickel-coupling-yield-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`nicolit-nickel-coupling-yield-percent-dataset.csv`](nicolit-nickel-coupling-yield-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
