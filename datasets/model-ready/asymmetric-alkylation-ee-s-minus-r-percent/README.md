# Asymmetric Alkylation ee (S-R, %) / 不对称烷基化 ee（S-R，%）
...
[`asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv`](asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv)；它的内容与不可变
[`v0.1.0-model-ready-preview-1`](https://github.com/thinktraveller/ord-datasets/releases/tag/v0.1.0-model-ready-preview-1)
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv`](asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 1,430 / 0 / 1,430 |
| 标签列 | `ee_s_minus_r_percent` |
| 标签含义与单位 | percentage points, S minus R |
| 标签选择规则 | `signed_selectivity_v1:s_config_imms_percentage` |
| 模型/基准就绪状态 | `generated_adapter_blocked` |
| 数据许可证 | `CC-BY-SA-4.0` |
| 相关论文 | [10.1038/s41467-023-42446-5](https://doi.org/10.1038/s41467-023-42446-5) |

> “model-ready”表示文件可以直接读取和建模，并不表示它已被接受为通用基准。建模或比较前，请结合上表的就绪状态与标签含义判断是否适合您的问题。

### 化学范围与筛选

面向光氧化还原/有机催化醛 alpha-不对称烷基化的对映选择性建模。标签为有符号 ee（S−R，百分比）而非产率；产物立体信息只用于标签判定，不作为模型特征。

- 本目标只使用下方列出的 ORD 物理数据源。
- 只有满足 `signed_selectivity_v1:s_config_imms_percentage` 的标签记录被纳入主表；不会对多个候选标签做未声明的平均或聚合。
- 实际排除情况：无额外排除。
...
| 试剂、催化剂、溶剂等 | `catalyst-1`、`catalyst-2`、`solvent-1`、`solvent-2`、`solvent-3`、`solvent-4`、`solvent-5` |
| 产物 | 无 |
| 条件 | 无 |
| 标签 | `ee_s_minus_r_percent` |
...
| 文件 | 用途 |
|---|---|
| [`asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv`](asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv`](asymmetric-alkylation-ee-s-minus-r-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
