# Flow Catechol Product 3 HPLC Response (%)

[中文](README-zh.md)

This is an ORD subset prepared for direct reaction modelling. Its main table is
[`flow-catechol-product-3-response-percent-dataset.csv`](flow-catechol-product-3-response-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`flow-catechol-product-3-response-percent-dataset.csv`](flow-catechol-product-3-response-percent-dataset.csv) |
| Included / excluded / input records | 1,227 / 0 / 1,227 |
| Label column | `product_3_yield_percent` |
...
Research-only modelling target for the HPLC percentage response of Product 3 in a transient flow reaction. Features include substrate, binary-solvent composition, temperature, and residence time; reliable run grouping is not yet available, so it is not a batch-model benchmark.
...
| File | Purpose |
|---|---|
| [`flow-catechol-product-3-response-percent-dataset.csv`](flow-catechol-product-3-response-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`flow-catechol-product-3-response-percent-dataset.csv`](flow-catechol-product-3-response-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
