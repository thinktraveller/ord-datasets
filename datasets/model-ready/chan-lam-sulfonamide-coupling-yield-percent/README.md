# Chan-Lam Sulfonamide-Coupling Yield (%) / Chan-Lam 磺酰胺偶联产率（%）
...
[`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv) |
| Included / excluded / input records | 9,602 / 30 / 9,632 |
...
| Associated publication | [10.26434/chemrxiv-2024-22jrq](https://doi.org/10.26434/chemrxiv-2024-22jrq) |
...
Yield modelling for Chan-Lam coupling of primary sulfonamides and boronic acids. The target covers diverse sulfonamides, boronic acids, copper catalysts, bases, and solvents, including replicate experiments for examining compatibility and robustness.
...
- Observed exclusions: `label_out_of_range`: 30 record(s)
...
| Reagents, catalysts, solvents, and other components | `reagent-1`, `catalyst-1`, `solvent-1`, `solvent-2` |
| Products | `product` |
...
| File | Purpose |
|---|---|
| [`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
...
- `ord_dataset-5c9a10329a8a48968d18879a48bb8ab2`: [data/5c/ord_dataset-5c9a10329a8a48968d18879a48bb8ab2.parquet](https://github.com/open-reaction-database/ord-data/blob/83f971f586f6ad18f358ae4ae99d045e94ed2066/data/5c/ord_dataset-5c9a10329a8a48968d18879a48bb8ab2.parquet)  (ORD revision `83f971f586f6ad18f358ae4ae99d045e94ed2066`; SHA-256 `a2ea1363cb895b590ae765e918f98f90d243d6caba763ed74543ae62e503b0b0`)
...
[`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv)；它的内容与不可变
[`v0.1.0-model-ready-preview-1`](https://github.com/thinktraveller/ord-datasets/releases/tag/v0.1.0-model-ready-preview-1)
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 9,602 / 30 / 9,632 |
| 标签列 | `yield_percent` |
...
| 数据许可证 | `CC-BY-SA-4.0` |
| 相关论文 | [10.26434/chemrxiv-2024-22jrq](https://doi.org/10.26434/chemrxiv-2024-22jrq) |

> “model-ready”表示文件可以直接读取和建模，并不表示它已被接受为通用基准。建模或比较前，请结合上表的就绪状态与标签含义判断是否适合您的问题。

### 化学范围与筛选

面向伯磺酰胺与硼酸的 Chan–Lam 偶联产率建模。体系覆盖多种磺酰胺、硼酸、铜催化剂、碱与溶剂，并含重复实验；适合分析底物兼容性和条件稳健性。

- 本目标只使用下方列出的 ORD 物理数据源。
- 只有满足 `unique_structured_scalar_no_aggregation` 的标签记录被纳入主表；不会对多个候选标签做未声明的平均或聚合。
- 实际排除情况：`label_out_of_range`：30 条
...
| 试剂、催化剂、溶剂等 | `reagent-1`、`catalyst-1`、`solvent-1`、`solvent-2` |
| 产物 | `product` |
...
| 文件 | 用途 |
|---|---|
| [`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`chan-lam-sulfonamide-coupling-yield-percent-dataset.csv`](chan-lam-sulfonamide-coupling-yield-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
