# Science HTE Relative LC Area Ratio / Science 高通量相对 LC 面积比
...
[`science-hte-relative-lc-area-ratio-dataset.csv`](science-hte-relative-lc-area-ratio-dataset.csv)；它的内容与不可变
[`v0.1.0-model-ready-preview-1`](https://github.com/thinktraveller/ord-datasets/releases/tag/v0.1.0-model-ready-preview-1)
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`science-hte-relative-lc-area-ratio-dataset.csv`](science-hte-relative-lc-area-ratio-dataset.csv) |
| 纳入 / 排除 / 输入记录 | 1,531 / 5 / 1,536 |
| 标签列 | `relative_product_lc_area_ratio` |
| 标签含义与单位 | dimensionless ratio (product LC area / internal-standard LC area) |
| 标签选择规则 | `unique_structured_scalar_no_aggregation` |
| 模型/基准就绪状态 | `partial_scope_generated` |
| 数据许可证 | `CC-BY-SA-4.0` |
| 相关论文 | [10.1126/science.1259203](https://doi.org/10.1126/science.1259203) |

> “model-ready”表示文件可以直接读取和建模，并不表示它已被接受为通用基准。建模或比较前，请结合上表的就绪状态与标签含义判断是否适合您的问题。

### 化学范围与筛选

面向 Pd 催化交叉偶联中多亲核试剂体系的相对 LC–MS 响应建模。标签为目标产物 AREA 与联苯内标 AREA 的比值，不是产率或转化率；适合比较亲核试剂、催化剂、碱和溶剂的相对响应规律。

- 本目标只使用下方列出的 ORD 物理数据源。
- 只有满足 `unique_structured_scalar_no_aggregation` 的标签记录被纳入主表；不会对多个候选标签做未声明的平均或聚合。
- 实际排除情况：`invalid_biphenyl_internal_standard_area`：5 条
...
| 试剂、催化剂、溶剂等 | `reagent-1`、`reagent-2`、`catalyst-1`、`solvent-1`、`solvent-2`、`solvent-3`、`solvent-4` |
| 产物 | 无 |
| 条件 | `reaction_time_s` |
| 标签 | `relative_product_lc_area_ratio` |
...
| 文件 | 用途 |
|---|---|
| [`science-hte-relative-lc-area-ratio-dataset.csv`](science-hte-relative-lc-area-ratio-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`science-hte-relative-lc-area-ratio-dataset.csv`](science-hte-relative-lc-area-ratio-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
