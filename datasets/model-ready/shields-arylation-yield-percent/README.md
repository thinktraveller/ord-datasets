# Shields Arylation Yield (%)

[中文](README-zh.md)

This is an ORD subset prepared for direct reaction modelling. Its main table is
[`shields-arylation-yield-percent-dataset.csv`](shields-arylation-yield-percent-dataset.csv). The CSV bytes are identical to `dataset.csv` in the immutable
...
| Item | Details |
|---|---|
| Main data file | [`shields-arylation-yield-percent-dataset.csv`](shields-arylation-yield-percent-dataset.csv) |
| Included / excluded / input records | 1,984 / 0 / 1,984 |
...
| Associated publication | [10.1038/s41586-021-03213-y](https://doi.org/10.1038/s41586-021-03213-y) |
...
Percent-yield modelling for arylation reactions. The data combine substrates, catalysts, bases, solvents, and continuous condition variables in an optimisation-trajectory-like design that can support Bayesian optimisation or condition recommendation studies.
...
| Reagents, catalysts, solvents, and other components | `reagent-1`, `catalyst-1`, `catalyst-2`, `solvent-1` |
| Products | `product` |
...
| File | Purpose |
|---|---|
| [`shields-arylation-yield-percent-dataset.csv`](shields-arylation-yield-percent-dataset.csv) | Analysis-ready main table; each row is an included reaction. |
...
1. Select a row in [`shields-arylation-yield-percent-dataset.csv`](shields-arylation-yield-percent-dataset.csv). Its CSV row number, counting the header as row 1, is the `csv_row_number` in `row-map.csv`.
