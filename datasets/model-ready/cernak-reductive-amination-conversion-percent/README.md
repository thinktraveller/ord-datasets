# Cernak Reductive-Amination Conversion (%) / Cernak 还原胺化转化率（%）

这是一个可直接用于反应建模的 ORD 数据子集。主表为
[`cernak-reductive-amination-conversion-percent-dataset.csv`](cernak-reductive-amination-conversion-percent-dataset.csv)；它的内容与不可变
...
| 项目 | 内容 |
|---|---|
| 主数据文件 | [`cernak-reductive-amination-conversion-percent-dataset.csv`](cernak-reductive-amination-conversion-percent-dataset.csv) |
...
面向 staurosporine 与醛或酮底物的小型化还原胺化转化率建模。体系兼具条件优化和底物库合成特征，适合评估从已见底物到新醛/酮底物的组别泛化能力。
...
| 文件 | 用途 |
|---|---|
| [`cernak-reductive-amination-conversion-percent-dataset.csv`](cernak-reductive-amination-conversion-percent-dataset.csv) | 可直接用于分析或建模的主表；每行是一条纳入的反应。 |
...
1. 在 [`cernak-reductive-amination-conversion-percent-dataset.csv`](cernak-reductive-amination-conversion-percent-dataset.csv) 中选定一行；其 CSV 行号（把表头算作第 1 行）就是 `row-map.csv` 的 `csv_row_number`。
