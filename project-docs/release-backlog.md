...
| F2 | project-owner | Review the rendered public Dataset Card/license/citation, explicitly authorize public HF Dataset creation and upload, then freeze an immutable revision and perform a clean remote readback. | Local RC `v0.2.0-corpus-preview-1` passed 41 corpus / 2,428,291 reaction / 53 source checks, 205 payload files / 13,127,476,889 bytes, and root `930a9111…563bc663`; see steps 31–32 full-corpus reports. Public write remains pending the explicit go decision and is followed by required remote readback. |
