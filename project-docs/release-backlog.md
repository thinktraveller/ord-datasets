...
| F2 | project-owner | Build a distinct public corpus RC for `thinktraveller/ord-processed-reaction-corpus`, version `v0.2.0-corpus-preview-1`, then freeze an immutable HF Dataset revision after corpus-specific validation. | The RC has exactly 41 corpus / 2,428,291 Reaction / 53 physical-source checks, 205 payload files / 13,127,476,889 bytes, an isolated root hash, and a clean HF Dataset readback. Public write remains pending the explicit go decision. |
